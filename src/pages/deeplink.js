...
export default function App () {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        const resources = await ky.get('/resources', { headers: { Authorization: 'Bearer ' + getLtik() } }).json()
  ...
  const submit = async () => {
    try {
      ...
      const form = await ky.post('/deeplink', { json: dataset[resource], headers: { Authorization: 'Bearer ' + getLtik() } }).text()
      $('body').append(form)
    } catch (err) {
      console.log(err)
