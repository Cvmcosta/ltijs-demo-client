...
export default function App () {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        const resources = await ky.get('http://localcvm.com:3000/resources', { headers: { Authorization: 'Bearer ' + getLtik() } }).json()
  ...
  const submit = async () => {
    try {
      ...
      const form = await ky.post('http://localcvm.com:3000/deeplink', { json: dataset[resource], headers: { Authorization: 'Bearer ' + getLtik() } }).text()
      $('body').append(form)
    } catch (err) {
      console.log(err)
